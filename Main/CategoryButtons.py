...
class CategoryButtons():
    ...
    def mousePressed(self):
        ...
        #Locatie op de x en y van de knoppen.
        standard_l = 250 <= mouseY <= 360
        knowledge_l = 400 <= mouseY <= 510
        math_l = 550 <= mouseY <= 660
        return_l = (40 <= mouseX <= 190) and (40 <= mouseY <= 160)
        
        #Zorgt dat functies worden uitgevoert als een knop is geklikt.
        if (standard_l or knowledge_l or math_l) and mouse_x and CategoryMain.current_screen == 'Categories':
            if standard_l:
                CategoryMain.set_screen('Standard')
            elif knowledge_l:
                CategoryMain.set_screen('Knowledge')
            else:
                CategoryMain.set_screen('Math')
        elif return_l:
